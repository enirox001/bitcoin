...
namespace {
template <typename T>
bool CanDeserializeLocally(const std::vector<uint8_t>& bytes)
{
    try {
...
public:
    ...
    UniValue PassUniValue(const UniValue& value)
    {
        return CallOnClientThreadAndWait([&](auto& client) { return client.passUniValue(value); });
    }

    CTransactionRef PassTransaction(const CTransactionRef& tx)
    {
        return CallOnClientThreadAndWait([&](auto& client) { return client.passTransaction(tx); });
    }

    bool DeserializeOutPoint(const std::vector<uint8_t>& bytes)
    {
        try {
            return CallOnClientThreadAndWait([&](auto& client) { return client.deserializeOutPoint(bytes); });
        } catch (const std::exception&) {
            return false;
        }
    }

    bool DeserializeScript(const std::vector<uint8_t>& bytes)
    {
        try {
            return CallOnClientThreadAndWait([&](auto& client) { return client.deserializeScript(bytes); });
        } catch (const std::exception&) {
            return false;
        }
    }

    bool DeserializeTransaction(const std::vector<uint8_t>& bytes)
    {
        try {
            return CallOnClientThreadAndWait([&](auto& client) { return client.deserializeTransaction(bytes); });
        } catch (const std::exception&) {
            return false;
        }
    }
    ...
    for (size_t i = 0; i < iterations; ++i) {
        try {
            CallOneOf(
                ...
                [&] {
                    UniValue value;
                    value.read(fuzzed_data_provider.ConsumeRandomLengthString(512));
                    assert(ipc.PassUniValue(value).write() == value.write());
                },
                [&] {
                    const auto mutable_tx = ConsumeTransaction(fuzzed_data_provider, /* prevout_txids= */ std::nullopt);
                    const CTransactionRef tx = MakeTransactionRef(mutable_tx);
                    const CTransactionRef result = ipc.PassTransaction(tx);
                    assert(*result == *tx);
                },
                [&] {
                    const auto bytes = ConsumeRandomLengthByteVector<uint8_t>(fuzzed_data_provider, 512);
                    assert(ipc.DeserializeOutPoint(bytes) == CanDeserializeLocally<COutPoint>(bytes));
                },
                [&] {
                    const auto bytes = ConsumeRandomLengthByteVector<uint8_t>(fuzzed_data_provider, 512);
                    assert(ipc.DeserializeScript(bytes) == CanDeserializeLocally<CScript>(bytes));
                },
                [&] {
                    const auto bytes = ConsumeRandomLengthByteVector<uint8_t>(fuzzed_data_provider, 2048);
                    assert(ipc.DeserializeTransaction(bytes) == CanDeserializeLocally<CMutableTransaction>(bytes));
                });
        } catch (const std::exception&) {
        }
    }
