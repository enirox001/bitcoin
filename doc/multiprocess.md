...
On Linux, vsock (AF_VSOCK) is supported as an IPC transport for communication between VM guests and host systems, useful for setups like Docker Desktop on macOS where Unix socket sharing is not possible. Use `vsock:<cid>:<port>` as the address format with `-ipcbind` and `-ipcconnect`.
